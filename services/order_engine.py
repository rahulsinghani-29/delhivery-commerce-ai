...
class OrderActionEngineService:
    ...
    def process_order(self, order: dict, merchant_config: dict) -> dict:
        ...
        risk_threshold = 0.4  # configurable
        ...
        # 2d. Confidence gate: suppress if confidence < 0.6
        if nba_result.get("confidence_score", 0.0) < 0.6:
        ...
        auto_cancel_threshold = merchant_config.get("auto_cancel_threshold", 0.9)
    ...
    def get_live_feed(
        self, merchant_id: str, risk_threshold: float = 0.4
    ) -> list[dict]:
        """Get recent orders, process each, return sorted by rto_score descending."""
        orders = get_recent_orders(self.db, merchant_id)
        if not orders:
            return []
        ...
        for order in orders:
            try:
                ...
                # Skip orders that fail processing
