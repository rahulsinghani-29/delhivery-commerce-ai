...
def update_permissions(
    ...
    risk_threshold = 0.4
    if body.auto_cancel_threshold is not None and body.auto_cancel_threshold <= risk_threshold:
        raise HTTPException(
            status_code=422,
            detail=f"auto_cancel_threshold ({body.auto_cancel_threshold}) must be greater than risk threshold ({risk_threshold})",
        )
    ...
    if existing:
        ...
        db.execute(
            ...
            (
                ...
                body.is_enabled,
                body.daily_cap or 500,
                body.hourly_cap or 100,
                body.auto_cancel_enabled or False,
                body.auto_cancel_threshold or 0.9,
...
@router.post("/orders/{order_id}/communications/trigger")
def trigger_communication(
    ...
    if result.get("error_message"):
        # Communication failed but not a server error — return the result with status info
        return result
    return result
