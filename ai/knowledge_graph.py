...
class RiskKnowledgeGraph:
    ...
    def get_risk_path(self, order: dict, db: sqlite3.Connection) -> RiskPath:
        ...
        if self.graph.has_edge(payment_id, cluster_id):
            ...
            # Compute network average RTO rate for this payment mode
            ...
            if cluster_rto > network_avg and network_avg > 0:
                ...
                factors.append(RiskFactor(
                    ...
                    data={"cluster_rto_rate": cluster_rto, "network_avg": network_avg,
                          "order_count": edge.get("order_count", 0)},
                ))
        ...
        if self.graph.has_edge(category_id, cluster_id):
            ...
            # Network average delivery rate for this category
            network_cat_avg = self._network_avg_delivery_for_category(order.get("category", ""))
        ...
        if address_quality < address_threshold:
            weight = min(0.3, (address_threshold - address_quality) * 0.6)
            factors.append(RiskFactor(
                factor_type="address_quality",
                description=(
                    f"Address quality score is {address_quality:.2f} "
                    f"(below threshold of {address_threshold})"
                ),
                weight=weight,
                data={"address_quality": address_quality, "threshold": address_threshold},
            ))
        ...
        if order.get("payment_mode") == "COD" and self.graph.has_edge(payment_id, cluster_id):
            ...
            if self.graph.has_edge(prepaid_id, cluster_id):
                prepaid_rto = self.graph.edges[prepaid_id, cluster_id].get("rto_rate", 0.0)
